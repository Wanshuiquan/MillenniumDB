...
Q11 = "DATA_TEST NAIVE ?e (person {oid - ?p > 15 and ?p - oid < 15})/ ((:knows {true} )/(person {oid - ?p > 15 and ?p - oid < 15}))*"
Q12 = "DATA_TEST NAIVE ?e (person {?p >= oid and ?q <= oid})/ ((:knows {true} )/(person {?p >= oid and ?q <= oid}))*"
Q13 = "DATA_TEST NAIVE ?e (person {?p >= oid and ?q <= oid and ?q - ?p <= 7})/ ((:knows {true} )/(person {?p >= oid and ?q <= oid and ?q - ?p <= 7}))*"
Q14 = "DATA_TEST NAIVE ?e (person {?p == oid and ?q == iid })/ ((:knows {true} )/(person {?q - iid <= 100 and iid - ?q <= 100 and 0.5 * oid + 100 < ?p }))*"
Q15 = """DATA_TEST NAIVE ?e (person {?q - iid + ?p - oid <= 100 and iid - ?q + ?p - oid <= 100 and iid - ?q + oid - ?p <= 100 and ?q - iid + oid - ?p <= 100})/ ((:knows {true} )/ 
(person {?q - iid + ?p - oid <= 100 and iid - ?q + ?p - oid <= 100 and iid - ?q + oid - ?p <= 100 and ?q - iid + oid - ?p <= 100 }))*"""

Q21 =  """
        DATA_TEST NAIVE ?e (person {oid - ?p > 15 and ?p - oid < 15})/ 
...
Q22 = """ 
       DATA_TEST NAIVE ?e (person {?p >= oid and ?q <= oid})/ 
...
Q23 = """ 
       DATA_TEST NAIVE ?e (person {?p >= oid and ?q <= oid and ?p - ?q <= 7})/ 
...
Q24 = """ 
       DATA_TEST NAIVE ?e (person {?p == oid and ?q == iid})/ 
...
Q25 = """ 
       DATA_TEST NAIVE ?e (person {?q - iid + ?p - oid <= 100 and iid - ?q + ?p - oid <= 100 and iid - ?q + oid - ?p <= 100 and ?q - iid + oid - ?p <= 100})/ 
...
Q31 = """
        DATA_TEST NAIVE ?e (person {oid - ?p > 15 and ?p - oid < 15})/ 
                ((:knows {oid - ?p > 15 and ?p - oid < 15} )/(tag {oid - ?p > 15 and ?p - oid < 15}))*/
                 ((:hasInterest {oid - ?p > 15 and ?p - oid < 15} )/(tagclass {oid - ?p > 15 and ?p - oid < 15}))
      
      """

Q32 = """ 
       DATA_TEST NAIVE ?e (person {?p >= oid and ?q <= oid})/ 
                ((:knows {?p >= oid and ?q <= oid} )/(tag {?p >= oid and ?q <= oid}))*/
                 ((:hasInterest {?p >= oid and ?q <= oid} )/(tagclass {?p >= oid and ?q <= oid}))
"""
...
Q33 = """ 
       DATA_TEST NAIVE ?e (person {?p >= oid and ?q <= oid and ?p - ?q <= 7})/ 
                ((:knows {true} )/(tag {?p >= oid and ?q <= oid and ?p - ?q  <= 7}))*/
                 ((:hasInterest {true} )/(tagclass {?p >= oid and ?q <= oid and ?p - ?q <= 7}))
"""


Q34 = """ 
       DATA_TEST NAIVE ?e (person {?p == oid and ?q == iid})/ 
                ((:knows {true} )/(tag {?q - iid <= 100 and iid - ?q <= 100 and 0.5 * oid + 100 < ?p}))*/
                 ((:hasInterest {true} )/(tagclass {?q - iid <= 100 and iid - ?q <= 100 and 0.5 * oid + 100 < ?p}))
"""

Q35 = """ 
       DATA_TEST NAIVE ?e (person {?q - iid + ?p - oid <= 100 and iid - ?q + ?p - oid <= 100 and iid - ?q + oid - ?p <= 100 and ?q - iid + oid - ?p <= 100})/ 
                ((:knows {true} )/(tag {?q - iid + ?p - oid <= 100 and iid - ?q + ?p - oid <= 100 and iid - ?q + oid - ?p <= 100 and ?q - iid + oid - ?p <= 100 }))*/
                 ((:hasInterest {true} )/(tagclass {?q - iid + ?p - oid <= 100 and iid - ?q + ?p - oid <= 100 and iid - ?q + oid - ?p <= 100 and ?q - iid + oid - ?p <= 100 }))
"""



Q41 = "DATA_TEST NAIVE ?e (person {oid - ?p > 15 and ?p - oid < 15 })/ (((:knows {true}) | (:hasInterest {true} ) | (hasType {true} ))/(person {oid - ?p > 15 and ?p - oid < 15}))*"
Q42 = "DATA_TEST NAIVE ?e (person {?p >= oid and ?q <= oid})/ (((:knows {true}) | (:hasInterest {true} ) | (hasType {true} ))/(person {?p >= oid and ?q <= oid}))*"
Q43 = "DATA_TEST NAIVE ?e (person {?p >= oid and ?q <= oid and ?p - ?q <= 7})/ (((:knows {true}) | (:hasInterest {true} ) | (:hasType {true} ))/(person {?p >= oid and ?q <= oid and ?p - ?q <= 7}))*"
Q44 = "DATA_TEST NAIVE ?e (person {?p == oid and ?q == iid})/ (((:knows {true}) | (:hasInterest {true} ) | (:hasType {true} ))/(person {?q - iid <= 100 and iid - ?q <= 100 and 0.5 * oid + 100 <= ?p}))*"
Q45 = "DATA_TEST NAIVE ?e (person {?q - iid + ?p - oid <= 100 and iid - ?q + ?p - oid <= 100 and iid - ?q + oid - ?p <= 100 and ?q - iid + oid - ?p <= 100})/ (((:knows {true}) | (:hasInterest {true} ) | (:hasType {true} ))/(person {?q - iid + ?p - oid <= 100 and iid - ?q + ?p - oid <= 100 and iid - ?q + oid - ?p <= 100 and ?q - iid + oid - ?p <= 100}))*"

Q51 = "DATA_TEST NAIVE ?e (person {oid - ?p > 15 and ?p - oid < 15})/ ((:knows {true} )/(person {oid - ?p > 15 and ?p - oid < 15}))+"
Q52 = "DATA_TEST NAIVE ?e (person {?p >= oid and ?q <= oid})/ ((:knows {true} )/(person {?p >= oid and ?q <= oid}))+"
Q53 = "DATA_TEST NAIVE ?e (person {?p >= oid and ?q <= oid and ?q - ?p <= 7})/ ((:knows {true} )/(person {?p >= oid and ?q <= oid and ?q - ?p <= 7}))+"
Q54 = "DATA_TEST NAIVE ?e (person {?p == oid and ?q == iid })/ ((:knows {true} )/(person {?q - iid <= 100 and iid - ?q <= 100 and 0.5 * oid + 100 < ?p }))+"
Q55 = """DATA_TEST NAIVE ?e (person {?q - iid + ?p - oid <= 100 and iid - ?q + ?p - oid <= 100 and iid - ?q + oid - ?p <= 100 and ?q - iid + oid - ?p <= 100})/ ((:knows {true} )/ 
(person {?q - iid + ?p - oid <= 100 and iid - ?q + ?p - oid <= 100 and iid - ?q + oid - ?p <= 100 and ?q - iid + oid - ?p <= 100 }))+"""
...
def icij_graph_query():
    ...
    kill_server(server)
    with open(ROOT_TEST_DIR / "result" / "ldbc01_naive_statistic.json", "w") as fb:
        json.dump(result, fb)

    with open(ROOT_TEST_DIR / "result" / "ldbc01_naive_result.json", "w") as fb:
