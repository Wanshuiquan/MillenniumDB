...
using namespace Paths::DataTest::LinearIntegerModel;
...
void DFSCheck::set_model(z3::solver& sat_solver) {
...
bool DFSCheck::check_constraints(const MacroStateInt& macro_state) {
...
bool DFSCheck::eval_check(uint64_t obj, MacroStateInt& macro_state, const std::string& formula) {
    update_value(obj);
    exploration_depth++;
...
void DFSCheck::_begin(Binding& _parent_binding) {
    ...
    auto start_macro_state = Paths::DataTest::LinearIntegerModel::init_macro_state(start_path_state, automaton.get_start());
...
const PathState* DFSCheck::expand_neighbors(MacroStateInt& macro_state) {
        ...
        while (macro_state.dfs_iter->next()) {
            ...
            for (auto& transition_node : automaton.from_to_connections[transition_edge.to]) {
                ...
                if (matched_label && check_value) {
                    ...
                    if (inserted.second && automaton.decide_accept(transition_node.to) && target_id == end_object_id.id) {
...
void DFSCheck::print(std::ostream& os, int indent, bool stats) const {
    ...
    os << std::string(indent, ' ') << "Paths::DATA::Integer::DFSCheck(path_var: " << path_var
       << ", start: " << start << ", end: " << end << ")\n";
}
