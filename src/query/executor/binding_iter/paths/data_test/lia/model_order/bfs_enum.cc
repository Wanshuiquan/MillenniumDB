...
bool BFSEnum::eval_check(uint64_t obj, MacroStateInt& macro_state, const std::string& formula) {
    update_value(obj);
    exploration_depth++;
...
const PathState* BFSEnum::expand_neighbors(MacroStateInt& macro_state) {
    if (iter->at_end()) {
