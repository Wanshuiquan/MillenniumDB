...
void DFSEnum::apply_reg_assigns(MacroState& macroState, const SMTTransition& trans) {
...
void DFSEnum::set_model(z3::solver& sat_solver, const MacroState& macroState)
{
...
bool DFSEnum::check_constraints(const MacroState& macroState)
{
...
bool DFSEnum::_next() {
    // Run preprocessor but don't abort if it fails — the main DFS should still try
