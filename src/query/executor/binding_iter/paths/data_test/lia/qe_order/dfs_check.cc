...
void DFSCheck::apply_reg_assigns(MacroState& macroState, const SMTTransition& trans) {
...
void DFSCheck::_begin(Binding& _parent_binding) {
    ...
    auto start_macro_state = Paths::DataTest::LIA::init_macro_state(start_path_state, automaton.get_start());
...
const PathState* DFSCheck::expand_neighbors(MacroState& macroState){
    // stop if automaton state has not outgoing transitions
    if (macroState.dfs_iter->at_end()) {
        macroState.dfs_transition = 0;
    ...
    while (macroState.dfs_transition < automaton.from_to_connections[macroState.automaton_state].size()) {
        auto &transition_edge = automaton.from_to_connections[macroState.automaton_state][macroState.dfs_transition];
        while (macroState.dfs_iter->next()) {
            // get the edge of edge and target
            uint64_t edge_id = macroState.dfs_iter->get_edge();
            uint64_t target_id = macroState.dfs_iter->get_reached_node();
            // progress with edges
            ...
            for (auto &transition_node: automaton.from_to_connections[transition_edge.to]) {
                ...
                if (matched_label && check_value) {
                    ...
                    if (new_state.second && automaton.decide_accept(transition_node.to) && target_id == end_object_id.id) {
                        return new_ptr;
                    }
                }
        ...
        macroState.dfs_transition++;
        if (macroState.dfs_transition < automaton.from_to_connections[macroState.automaton_state].size()) {
            set_iter(macroState);
        }
...
bool DFSCheck::_next() {
    ...
    if (first_next) {
        ...
        if (current_state. path_state->node_id == end_object_id && automaton.decide_accept(current_state.automaton_state)
            && mark_emitted(current_state.path_state)) {
    ...
    while (!open.empty()) {
        ...
        if (reached_final_state != nullptr && mark_emitted(reached_final_state)) {
