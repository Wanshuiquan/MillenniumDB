...
static int64_t extract_int64(const z3::expr& value) {
    int64_t result = 0;
    if (value.is_numeral_i64(result)) {
        return result;
    }
    // Fallback: try as_double and cast
    return static_cast<int64_t>(value.as_double());
}
...
int MacroState::update_bound(std::tuple<Bound, int64_t, z3::expr> bound) {
    ...
    switch (type) {
        case Gt: {
            int64_t new_bound = extract_int64(value);
        ...
        case Le: {
            ...
            if (upper_bounds.find(key) == upper_bounds.end()) {
                ...
                int64_t old_bound = upper_bounds[key];
        ...
        case Ge: {
            ...
            if (lower_bounds.find(key) == lower_bounds.end()) {
                ...
                int64_t old_bound = lower_bounds[key];
...
void MacroState::initialize_from(const MacroState &other) {
    ...
    gt_vals = other.gt_vals;
    lt_vals = other.lt_vals;
