...
bool DFSCheck::check_constraints(const MacroStateReal& macro_state) {
    z3::solver sat_solver = solver;
    if (!macro_state.check_constraints(sat_solver, entailment_pipeline, true)) {
        return false;
    }
...
const PathState* DFSCheck::expand_neighbors(MacroStateReal& macro_state) {
        ...
        while (macro_state.dfs_iter->next()) {
            ...
            for (auto& transition_node : automaton.from_to_connections[transition_edge.to]) {
                ...
                if (matched_label && check_value) {
                    ...
                    if (inserted.second
                        && automaton.decide_accept(transition_node.to)
...
void DFSCheck::print(std::ostream& os, int indent, bool stats) const {
    ...
    os << std::string(indent, ' ') << "Paths::DATA::Real::DFSCheck(path_var: " << path_var
       << ", start: " << start << ", end: " << end << ")\n";
}
