   ...
   // Macro State to store the formulas
    struct MacroStateReal {
        ...
        SMT::CheckStatus check_constraints(
                z3::solver& solver,
                SMT::Real::AIEntailmentPipeline& entailment_pipeline) const
        {
            const auto oracle_status = entailment_pipeline.check_sat_status(
                    collected_expr_bv, collected_expr_int);
...
template<>
struct std::hash<Paths::DataTest::Real::MacroStateReal> {
    std::size_t operator() (const Paths::DataTest::Real::MacroStateReal & lhs) const {
        std::size_t seed = 0;
