...
namespace Paths::DataTest::LinearRealModel {
   ...
   // Macro State to store the formulas
    ...
    {
        return MacroStateReal {path, state, expr_int, expr_bv};
    }
...
template<>
struct std::hash<Paths::DataTest::LinearRealModel::MacroStateReal> {
    std::size_t operator() (const Paths::DataTest::LinearRealModel::MacroStateReal & lhs) const {
        std::size_t seed = 0;
