...
class DFSEnum : public BindingIter {
    ...
    std::unique_ptr<DFSPreEnum> preprocessor;
...
public:
    ...
    inline void set_iter(const MacroStateReal& state) {
        auto& transition = automaton.from_to_connections[state.automaton_state][state.dfs_transition];
        auto id = QuadObjectId::get_named_node(transition.type);
        state.dfs_iter = provider->get_iter(id.id, transition.inverse, state.path_state->node_id.id);
        idx_searches++;
    }
};
