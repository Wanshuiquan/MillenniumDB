...
void NaiveDFSCheck::apply_reg_assigns(SearchState& searchState, const SMTTransition& trans) {
...
void NaiveDFSCheck::_begin(Binding& _parent_binding) {
    ...
    // init the start node
    auto* start_path_state =  add_path_state(start_object_id,ObjectId::get_null(),ObjectId::get_null(), false,nullptr);

    // Populate attributes for the start node (needed for register assignments)
    // Pre-declare all attribute variables with Z3 before any parsing
...
const SearchState* NaiveDFSCheck::expand_neighbors(SearchState& search_state){
...
bool NaiveDFSCheck::_next() {
    ...
    if (first_next) {
        ...
        if (current_state.path_state-> node_id == end_object_id && automaton.decide_accept(current_state.automaton_state) ) {
            ...
            auto path_id = path_manager.set_path(reinterpret_cast<const Paths::DataTest::LRA_SubsetOrder::PathState*>(current_state.path_state), path_var);
    ...
    while (!open.empty()) {
        ...
        if (reached_final_state != nullptr) {
            auto path_id = path_manager.set_path(reinterpret_cast<const Paths::DataTest::LRA_SubsetOrder::PathState*>(reached_final_state->path_state), path_var);
            parent_binding->add(path_var, path_id);
            for (const auto& ele: vars){
                parent_binding->add(ele.first, QuadObjectId::get_value(to_string(ele.second)));
            }
...
void NaiveDFSCheck::_reset() {
    ...
    auto* start_search_state =  add_path_state(start_object_id,
        ObjectId::get_null(),
        ObjectId::get_null(),
        false,
        nullptr);
...
void NaiveDFSCheck::print(std::ostream& os, int indent, bool stats) const
{
    if (stats) {
        if (stats) {
    ...
    os << std::string(indent, ' ') << "Paths::DATA_Naive::DFSCheck(path_var: " << path_var
       << ", start: " << start << ", end: " << end << ")";
}
