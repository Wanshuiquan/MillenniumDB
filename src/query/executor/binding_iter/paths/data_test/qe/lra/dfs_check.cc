...
bool DFSCheck::eval_check(uint64_t obj, MacroState& macroState, const std::string& formula) {
    ...
    update_value(obj);
    exploration_depth++;
    if (!data_test_formula_attributes_complete(formula, attributes, real_attributes, string_attributes, boolean_attributes)) {
        return false;
    }
    // Initialize context
...
void DFSCheck::print(std::ostream& os, int indent, bool stats) const
{
    ...
    os << std::string(indent, ' ') << "Paths::DATA::DFSCheck(path_var: " << path_var
