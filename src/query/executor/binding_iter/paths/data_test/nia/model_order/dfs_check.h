...
public:
    ...
    bool mark_emitted(const PathState* path_state) {
        std::vector<uint64_t> key;
        for (auto current = path_state; current != nullptr; current = current->prev_state) {
            key.push_back(current->node_id.id);
            key.push_back(current->type_id.id);
            key.push_back(current->edge_id.id);
            key.push_back(current->inverse_dir ? 1 : 0);
        }
        return emitted_paths.emplace(std::move(key)).second;
    }
