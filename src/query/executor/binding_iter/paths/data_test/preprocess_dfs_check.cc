...
#include "preprocess_dfs_check.h"
...
void DFSPreCheck::_begin(Binding& _parent_binding)
{
...
const PreSearchState* DFSPreCheck::expand_neighbors(PreSearchState& search_state)
{
...
void DFSPreCheck::print(std::ostream& os, int indent, bool stats) const
{
    if (stats)
    {
        if (stats)
    ...
    os << std::string(indent, ' ') << "Paths::DATA::DFSPreCheck(path_var: " << ", start: " << start << ", end: "  << end<< ")";
}
