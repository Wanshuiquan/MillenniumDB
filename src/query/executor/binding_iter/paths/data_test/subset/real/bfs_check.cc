        ...
        while (iter->next()) {
            ...
            for (auto &transition_node: automaton.from_to_connections[transition_edge.to]) {
                ...
                if (matched_label) {
                    ...
                    z3::ast_vector_tpl<z3::expr> visited_constraints(
                            *get_smt_ctx().get_context());
                    ...
                    substitution(edge_id, visited_constraints, transition_edge.property_checks, successor_state.reg_vals);
                    substitution(target_id, visited_constraints, transition_node.property_checks, successor_state.reg_vals);
                    auto [state, inserted] = add_search_state(
                        ObjectId(target_id),
