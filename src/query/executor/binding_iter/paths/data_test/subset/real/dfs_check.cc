        ...
        while (search_state.dfs_iter->next()) {
            ...
            for (auto &transition_node: automaton.from_to_connections[transition_edge.to]) {
                ...
                if (matched_label) {
                    ...
                    for (const auto& formula : search_state.formulas) {
                        visited_constraints.push_back(formula);
                    }
...
void NaiveDFSCheck::_reset() {
    ...
    PathState start_search_state(start_object_id,
        ObjectId::get_null(),
        ObjectId::get_null(),
        false,
        nullptr);
